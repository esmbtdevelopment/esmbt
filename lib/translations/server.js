...
export const getTranslations = cache(
    unstable_cache(
        async (locale) => {
            try {
                console.log(`[Cache] Firestore READ - Translations: ${locale}`);

                // Fetch from Firestore
                const result = await getTranslationsByLocale(locale);

                if (result.success) {
                    console.log(`[Cache] Successfully loaded ${Object.keys(result.translations).length} translation keys for ${locale}`);
        ...
        ['translations'], // Cache key - Next.js automatically includes function parameters (locale)
        {
